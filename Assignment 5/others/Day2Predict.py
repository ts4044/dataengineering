        ...
        with engine.begin() as conn:
            ...
            conn.execute(
                text(
                    "INSERT INTO "
                    + curr
                    + "_output (window, balance, return_val, predicted_return, position)"
                      " VALUES (:window, :balance, :return_val, :predicted_return, :position);"
                ),
                [
                    {
                        "window": window,
...
def main(currency_pairs):
    ...
    # Initialize database
    engine = create_engine("sqlite:///../sqlite/prediction.db", echo=False, future=True)
...
def generate_csv():
    ...
    for curr in currency_pairs:
        ...
        db_df = pd.read_sql_query(sql, conn)
        path = "outputs/" + curr + ".csv"
        db_df.to_csv(path, index=False)


# In[ ]:

def temp(currency_pairs, currencyCheck, window, long_currency):
    # Load the saved models into a dictionary
    saved_models = load_saved_models(currency_pairs)
    # Load the cutoff values into a dictionary
    cutoff_divider_values = load_divider_values(currency_pairs)
    ...
    for curr in currency_pairs:
        ...
        df = pd.read_csv("./results/{}.csv".format(curr))
        df2 = df.copy()

        df = df.iloc[(window * 10) - 10:(window * 10)]
        # return_val stores the actual sum of last 10 return_val
        return_val = df["return_val"].sum()
